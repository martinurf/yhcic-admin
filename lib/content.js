...
export async function requestPublish(table, id, path) {
  ...
  /* is_private only exists on projects/announcements — members has no
     such column, so it's only selected for tables that have it. */
  const hasPrivacy = table === "projects" || table === "announcements";
  const { data: row } = await supabase
    .from(table)
    .select(hasPrivacy ? "published, is_private" : "published")
    .eq("id", id)
    .maybeSingle();
  if (!row) return { error: "Not found." };
  if (row.published) return { error: "Already published." };
  /* A private draft is a personal space, not something anyone else has
     seen yet — sending it straight to public review would skip the
     point of drafting privately in the first place. Share it with the
     workspace first (RLS also enforces this isn't bypassable). */
  if (hasPrivacy && row.is_private) return { error: "This is a private draft — share it with the team first, then request publish." };
